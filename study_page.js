...
// 언어 표시 모드 관련 DOM 요소는 이제 아이콘화되어 통합 컨트롤 바에 포함됨
// const foreignOverlay = document.getElementById('foreignOverlay'); // 아래에서 돔 참조를 밖으로 빼 단어 박스와 연동
// const meaningOverlay = document.getElementById('meaningOverlay'); // 아래에서 돔 참조를 밖으로 빼 단어 박스와 연동
...
// 표시 모드 토글 아이콘 버튼 및 관련 DOM 요소 (새로 추가)
...
/**
 * 현재 라디오 버튼 설정에 따라 오버레이를 적용/제거합니다.
 */
function applyOverlayState() {
    // 이제 라디오 버튼 직접 참조가 아닌, localStorage에 저장된 displayMode 값을 기준으로 함
    ...
    if (selectedMode === 'AB') { 
        ...
        console.log('[applyOverlayState] Mode AB: Meaning overlay visible.');
        ...
        console.log('[applyOverlayState] Mode BA: Foreign overlay visible.');
    }
}
...
swipeInteractionArea.addEventListener('touchend', (e) => {
    ...
    if (isSwiping && Math.abs(deltaX) > swipeThreshold) { 
        if (deltaX > 0) { // 오른쪽으로 스와이프 (이전 단어)
            prevBtn.click();
            console.log('Swipe detected: Previous word');
        } else { // 왼쪽으로 스와이프 (다음 단어)
            nextBtn.click();
...
prevBtn.addEventListener('click', () => {
    const currentWords = getWordsForCurrentMode();
    // 현재 Day의 첫 단어일 경우 이전 Day로 이동 시도
    if (currentIndex === 0 && currentStudyDay > 1) {
        currentStudyDay--;
        localStorage.setItem('currentStudyDay', currentStudyDay); // Day 변경 저장
        currentIndex = getWordsForCurrentMode().length - 1; // 이전 Day의 마지막 단어로 이동
        displayWord();
    } else if (currentIndex > 0) { // 현재 Day 내에서 이동
        currentIndex--;
        displayWord();
    } else {
        statusMessageElem.textContent = '자료의 처음입니다.';
...
nextBtn.addEventListener('click', () => {
    ...
    // 현재 Day의 마지막 단어일 경우 다음 Day로 이동 시도
    ...
    if (currentIndex === currentWords.length - 1 && currentStudyDay < totalDaysInSchedule) {
        currentStudyDay++;
        localStorage.setItem('currentStudyDay', currentStudyDay); // Day 변경 저장
        currentIndex = 0; // 다음 Day의 첫 단어로 이동
        displayWord();
    } else if (currentIndex < currentWords.length - 1) { // 현재 Day 내에서 이동
        currentIndex++;
        displayWord();
    } else {
        statusMessageElem.textContent = '자료의 마지막입니다.';
